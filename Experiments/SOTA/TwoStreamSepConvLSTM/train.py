...
#  f"C:/Users/ajayt/OneDrive/Desktop/Main P/other/TwoStreamSepConvLSTM_ViolenceDetection/rwf2000/processed"
...
def train(args):
    ...
    if preprocess_data:
        if dataset == 'rwf2000':
            ...
            convert_dataset_to_npy(src=f'{dataset}/RWF-2000', dest=destination, crop_x_y=None, target_frames=vid_len, frame_size= dataset_frame_size)
        else:
            ...
            os.mkdir(os.path.join(dataset,'processed'))
            convert_dataset_to_npy(src='{}/videos'.format(dataset),dest=destination, crop_x_y=crop_dark[dataset], target_frames=vid_len, frame_size= dataset_frame_size )

    train_generator = DataGenerator(directory = destination + '/train',
    ...
    test_generator = DataGenerator(directory = destination + '/test',
                                    batch_size = batch_size,
                                    data_augmentation = False,
                                    shuffle = False,
                                    one_hot = one_hot,
...
def main():
    ...
    parser.add_argument('--dest', type=str, help="Directory path of processed dataset")
